/* Arrakis "glass" button fluid effect — WebGL2 worker (1:1 port).
   Protocol: {type:"start"|"trail"|"trailEnd"|"stop"} in,
             {type:"ready"} / {type:"frame", id, bitmap} out. */
(() => {
  "use strict";

  let loc; // uniform/attribute locations
  const post = (msg, transfer) => self.postMessage(msg, transfer);

  let canvas = null;   // OffscreenCanvas
  let gl = null;       // WebGL2 context
  let prog = null;     // shader program
  let buf = null;      // fullscreen quad buffer
  let W = 0, H = 0;    // canvas pixel size
  let running = false;
  let id = "";         // instance id ("glass-N")
  let timer = null;
  let t0 = 0;          // intro start timestamp
  let dirX = -1, dirY = 0;
  let tone = 0;        // 0 = light button bg, 1 = dark button bg
  let trail = [];      // [{x, y, t}] newest first
  const trailArr = new Float32Array(42); // 14 * 3
...
void main() {
  ...
  const frame = () => {
    if (!running || !gl || !canvas || !prog) return;
  ...
  self.onmessage = (e) => {
    ...
    switch (n.type) {
      case "start": {
        ...
        id = n.id;
        if (canvas) { W = n.canvasW; H = n.canvasH; canvas.width = n.canvasW; canvas.height = n.canvasH; }
        t0 = performance.now();
        dirX = typeof n.dirX === "number" ? n.dirX : -1;
        dirY = typeof n.dirY === "number" ? n.dirY : 0;
        tone = typeof n.tone === "number" ? n.tone : 0;
        trail = [];
        running = true;
        frame();
        break;
